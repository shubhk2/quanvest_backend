...
def get_cg_committee_meetings(company_number: int) -> Dict[str, Any]:
    """
    Returns Committee Meetings details for a given company_number excluding 'id' and 'company_no'.
    """
    conn = connect_to_db()
    ...
    try:
        ...
        # Fetch committee meetings data excluding id and company_no
        cursor.execute(
            """
            SELECT committee_name, meeting_date, meeting_type, quorum_met,
                   directors_on_meeting_date, directors_present,
                   independent_directors_present, gap_between_meetings_days
            FROM public.cg_committee_meetings
            WHERE company_no = %s
            ORDER BY id
            """,
            (company_number,),
        ...
        logger.error(f"Error in get_cg_committee_meetings: {e}", exc_info=True)
        return {"error": str(e)}
    finally:
        cursor.close()
        conn.close()
